...
const DashboardEmptyState = ({ c_date }: { c_date: Date }) => {
	...
	useQuery({
		...
		queryFn: async () => {
			const date = new Date()
			const userDate = new Date(c_date)
			const diffMs = date.getTime() - userDate.getTime();

			const diffTime = Math.floor(diffMs / (1000 * 60))
			const localData = JSON.parse(localStorage.getItem("telemetry-called") || "{}")


			if ((diffTime < 3 && !localData.telemetryCalled)) {
				const response = await fetch("https://api.ipify.org")
				const data = await response.text()
				await client.project.ipInfo.$get({ c_date, data })
				localStorage.setItem("telemetry-called", JSON.stringify({ telemetryCalled: true }))
